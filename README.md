...
AWFUL takes inspiration from many programming language designs and concepts, but is not really informed by them and combines them in the most awful way (e.g. use of terms may differ from what existing languages mean).

The result is a fun little programming language interpreter that allows to write stack-based programs with a reduced set of keywords and language concepts.
...
- extensive internal state machine logging `--debug`, `--items`, `--stats`, `debug on/off/here/assert`
- nested `include`
...
- the internal data representation for all AWFUL data types for symmetry's sake are *always* dictionaries with keys as static Python strings (symbols from the token stream, or stringified integers), and mapped values as integers. For built-in types and standard library types, the 'type' points to a numeric type identifier
  - `nil` is a singleton data type with no map key except `type` set to 0
